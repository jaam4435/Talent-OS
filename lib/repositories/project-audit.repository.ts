...
import type { ProjectAuditEntry } from '@/modules/project/types'
...
export class ProjectAuditRepository extends BaseRepository {
  async record(input: ProjectAuditInput): Promise<void> {
    const { error } = await this.ctx.supabase.from('project_audit_logs').insert({
  ...
  ): Promise<PaginatedResult<ProjectAuditEntry>> {
