...
import type { AssignmentAuditEntry } from '@/modules/assignment/types'
...
export class AssignmentAuditRepository extends BaseRepository {
  async record(input: AssignmentAuditInput): Promise<void> {
    const { error } = await this.ctx.supabase.from('assignment_audit_logs').insert({
  ...
  ): Promise<PaginatedResult<AssignmentAuditEntry>> {
