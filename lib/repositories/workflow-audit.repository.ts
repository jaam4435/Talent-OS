...
import type { WorkflowAuditEntry } from '@/modules/workflow-engine/types'
...
export class WorkflowAuditRepository extends BaseRepository {
  async record(input: WorkflowAuditInput): Promise<void> {
    const { error } = await this.ctx.supabase.from('workflow_audit_logs').insert({
