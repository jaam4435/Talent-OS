...
import type { TalentAuditEntry } from '@/modules/talent/types'

export interface TalentAuditInput {
  tenant_id: string
...
export class TalentAuditRepository extends BaseRepository {
  async record(input: TalentAuditInput): Promise<void> {
    const { error } = await this.ctx.supabase.from('talent_audit_logs').insert({
  ...
  ): Promise<PaginatedResult<TalentAuditEntry>> {
    const { limit, offset, page } = this.paginate(options)
    let query = this.ctx.supabase
      .from('talent_audit_logs')
      .select(
